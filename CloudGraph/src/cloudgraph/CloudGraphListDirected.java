...
public class CloudGraphListDirected {
	...
	private Map<LocationItem, List<Edge>> adj;
	private int numEdges;
	private List<LocationItem> Mark;
	...
	public CloudGraphListDirected(String graphName) {
		this.graphName = graphName;
		adj = new HashMap<LocationItem, List<Edge>>();
		Mark = new LinkedList<LocationItem>();
		numEdges = 0;
		
		
		dbGraphAccess = new DBAccessGraphDynamoDB();
		dbGraphAccess.createTable(graphName);
		
	}
	
	public LocationItem setVertex(int x, int y) {
		LocationItem locationItem = new LocationItem(x, y, null);
		int i = r.nextInt(animals.length);
		locationItem.setType(animals[i]);
		
		dbGraphAccess.addGraphVertex(animals[i], null, locationItem.getJson().toJSONString(), graphName);
		
		Mark.add(locationItem);
		return locationItem;
	}
	
	public void SetEdgeDirected(LocationItem v1, LocationItem v2, int weight) {
		if (adj.get(v1) == null) {
		...
		dbGraphAccess.updateItem(graphName, v1.getType(), arr.toJSONString());
	...
	public Set<LocationItem> getPoints() {
		Map<LocationItem, List<Edge>> pointsAndEdges = dbGraphAccess.getCloudVerticesAndEdges(graphName);
		
		for (LocationItem item : pointsAndEdges.keySet()) {
			
			for (Edge e : pointsAndEdges.get(item)) {
				
				LocationItem v1 = e.v1();
				LocationItem v2 = e.v2();
	...
	public List<Edge> neighbors(LocationItem i) {
	...
	public List<Edge> getEdges() {
		...
		for (LocationItem item : adj.keySet()) {
